...
export default function MassReajusteDrawer({ acordo, influencers, onClose, onSuccess }: Props) {
  ...
  const validationHint = !canSubmit && (
    selected.size === 0 ? 'Selecione pelo menos 1 influencer' :
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/gerente/acordos/${acordo.id}/reajuste-massa`, {
        method: 'POST',
        ...
        <div className="flex flex-col gap-2">
          ...
          {afetaveis.length === 0 ? (
            <div className="rounded-xl p-4 text-sm text-center text-[var(--color-on-surface-variant)] opacity-60"
              ...
              Nenhum influencer tem contrato neste acordo ainda.
            </div>
          ) : (
            <div
