...
export default function GerentesClient({ rows, totalPendente, totalPago, totalAfiliados }: Props) {
  ...
  return (
    <div className="animate-fade-in flex flex-col gap-6">
      {/* Header */}
      <div>
        <h1 className="text-display-lg text-[var(--color-on-surface)]">Pagamentos — Gerentes</h1>
        <p className="text-body-md text-[var(--color-on-surface-variant)] mt-0.5">
          Volume de saques por gerente
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        ...
        <StatCard label="Afiliados"  value={String(totalAfiliados)} sub={`${rows.length} gerente${rows.length !== 1 ? 's' : ''}`} icon="manage_accounts" accent="#0275F3" />
      </div>
      ...
      {filtered.length === 0 && rows.length === 0 ? (
        <div className="glass-card rounded-xl py-16 flex flex-col items-center gap-3">
          <span className="material-symbols-outlined text-[48px] opacity-25" style={{ color: 'var(--color-on-surface-variant)' }}>manage_accounts</span>
          <p className="text-sm font-medium" style={{ color: 'var(--color-outline)' }}>Nenhum gerente cadastrado</p>
        </div>
      ) : (
      <div className="glass-card rounded-xl overflow-hidden">
        ...
        <table className="w-full text-sm">
          <thead style={{ background: 'var(--color-surface-container-low)', borderBottom: '1px solid var(--color-outline-variant)' }}>
            <tr>
              {['Gerente', 'Afiliados', 'Saques', 'Pendente', 'Pago', 'Chave PIX'].map(h => (
