...
export default async function AdminFinanceiroIntermediariosPage() {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user || user.app_metadata?.role !== 'admin') redirect('/dashboard')

  const [intermediarios, influencers] = await Promise.all([
    prisma.public_users.findMany({
      where: {
        ativo: true,
        user_roles: { some: { role: 'INTERMEDIARIO' as never, ativo: true } },
      },
      select: { id: true, nome_completo: true, email: true, pix_key: true, pix_key_type: true },
      orderBy: { nome_completo: 'asc' },
    }),
    prisma.public_users.findMany({
      where: { ativo: true, id_intermediario: { not: null } },
      select: {
        id: true,
  ...
  const rows = intermediarios.map(inter => {
    const myInfluencers = influencers.filter(u => u.id_intermediario === inter.id)
    let pendente = 0
    let pago = 0
    let countSaques = 0
    ...
    return {
      id: inter.id,
      nome: inter.nome_completo,
      email: inter.email,
      pix_key: inter.pix_key ?? null,
      pix_key_type: inter.pix_key_type ?? null,
