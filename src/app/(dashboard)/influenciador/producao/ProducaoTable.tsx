...
export function ProducaoTable({ rows, totals, myId }: ProducaoTableProps) {
  const columns: Column<ProducaoRow>[] = [
    ...
    },
    {
      ...
      cell: (row) => <span style={{ color: '#22D3A5' }}>{fmt(n(row.receita_total))}</span>
    }
  ]
  ...
  const footer = (
    <tr>
      ...
      <td className="px-5 py-4 text-sm font-bold tabular-nums text-right" style={{ color: '#22D3A5' }}>{fmt(totals.receita_total)}</td>
    </tr>
  )
