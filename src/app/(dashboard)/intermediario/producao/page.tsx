...
export default async function IntermediarioProducaoPage({
  searchParams,
}: {
  searchParams: Promise<{ periodo?: string; gerente?: string; casa?: string; agrupamento?: string; dataInicio?: string; dataFim?: string }>
}) {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) redirect('/login')

  const { periodo = '30', gerente: filterGerente, casa: filterCasa, agrupamento = 'diaria', dataInicio, dataFim } = await searchParams
  ...
  if (myId) {
    ...
    if (userRole) {
      ...
      if (contractIds.length > 0) {
        // 3. Buscar dados da view vw_producao_intermediario
        ...
        rows = viewRows.map(r => ({
          ...
          custo_repassado_cpa: r.custo_repassado_cpa?.toString() ?? '0',
          custo_repassado_revshare: r.custo_repassado_revshare?.toString() ?? '0',
  ...
  const summaryCards = [
    ...
    { label: 'Custo Rep.', value: fmt(totals.custo_repassado_total), icon: 'payments', color: '#F43F5E' },
    ...
    <div className="animate-fade-in flex flex-col gap-6 min-w-0 w-full max-w-full">
      <div>
        ...
        <p className="text-body-md text-[var(--color-on-surface-variant)] mt-1">
          Analytics de produção da rede (intermediário → gerentes → influencers)
        </p>
      </div>
        ...
        <div className="glass-card p-5 rounded-2xl flex flex-col gap-3">
          ...
          <div className="flex items-center gap-6 text-sm flex-wrap">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[var(--color-on-surface-variant)]">Custo Repassado:</span>
